...
- **Lo que ya vigila una comprobación**: `eslint`, `oxlint`, `prettier`, `tsc`, las pruebas de backend y de frontend, `openapi:check`, las comprobaciones de `verificar-docs.mjs` y los scripts `probar-*` de `scripts/`. Si un script ya lo muerde, un modelo mirándolo es gasto.
