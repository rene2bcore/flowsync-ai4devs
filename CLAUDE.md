...
Tests (Japa). Dos suites declaradas en `adonisrc.ts`: `unit` (`tests/unit/**/*.spec.ts`, timeout 2s) y `functional` (`tests/functional/**/*.spec.ts`, timeout 30s). Hoy hay **81 pruebas functional**: 26 de `auth`, 41 de `tasks`, 6 de errores, 2 de aislamiento de la base y 6 que fijan los nombres de regla que el frontend traduce (H-05). **`tests/unit/` no existe.** Qué escenario cubre cada una en `docs/trazabilidad.md`.
